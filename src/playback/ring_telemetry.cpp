...
std::optional<uint64_t> RingTelemetry::observe(size_t queued, uint64_t nowMs) {
    ...
    if (markMs_ == 0) {
        ...
        log::info(log::Area::Pb, "ring 10s: cur={} min={} max={} samples", queued, min_, max_);
    ...
    if (queued == 0) {
        if (!starvedMs_) {
            starvedMs_ = nowMs;
            log::warn(log::Area::Pb, "ring starved (0 samples)");
        }
    } else if (starvedMs_) {
        log::info(log::Area::Pb, "ring recovered after {} ms", nowMs - starvedMs_);
        starvedMs_ = 0;
    }
    return window;
}
